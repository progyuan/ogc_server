...
class SpatialLite (DataSource):
    ...
    def select(self, action):
        ...
        if action.id is not None:
            sql = "SELECT AsText(ST_Transform(%s, %d)) as fs_text_geom, " % (self.geom_col, int(self.srid_out))
            ...
            if action.attributes:
                ...
                for key, value in action.attributes.items():
                    if isinstance(value, dict):
                        ...
                        attrs[key] = value
            if action.bbox:
                filters.append("Intersects(ST_Transform(BuildMBR(%f, %f, %f, %f, %s), %s), geometry)" % (tuple(action.bbox) + (self.srid_out,) + (self.srid,)))
