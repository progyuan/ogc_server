...
class PostGIS (DataSource):
    ...
    def __init__(self, name, srid = 4326, srid_out = 4326, fid = "gid", geometry = "geom", fe_attributes = 'true', order = "", attribute_cols = '*', writable = True, encoding = "utf-8", hstore = 'false', hstore_attr = "", **args):
    ...
    def select (self, action):
        ...
        if action.id is not None:
            ...
            sql = "SELECT ST_AsText(ST_Transform(%s, %d)) as fs_text_geom, " % (self.geom_col, int(self.srid_out))
            #sql = "SELECT ST_AsText(%s) as fs_text_geom, "  % self.geom_col
            if hasattr(self, 'ele'):
                sql += "%s as ele, " % self.ele
            if hasattr(self, 'version'):
                sql += "%s as version, " % self.version
            ...
            try:    
                ...
                if action.maxfeatures is None:
                    action.maxfeatures = 10000
                result = cursor.fetchmany(action.maxfeatures)
