...
WFS_NAMESPACES = {'ows':"http://www.opengis.net/ows",
...
class Request (object):
    ...
    def parse(self, params, path_info, host, post_data, request_method, format_obj = None):
        ...
        try:
            ...
            for layer in self.service.datasources:
                if not layer in self.datasources:
        ...
        if not post_data is None and len(post_data)>0:
            post_data = post_data.replace('xmlns:=','xmlns:my=')
            params = self.get_params_from_xml(post_data)
        
        action = Action()
        
        if request_method == "POST" and params.has_key('REQUEST') and params['REQUEST']=='GetFeature':
            action = self.get_select_action(path_info, params)
            
        elif request_method == "GET" or (request_method == "OPTIONS" and (post_data is None or len(post_data) <= 0)):
            action = self.get_select_action(path_info, params)
    ...
    def get_params_from_xml(self, xml):
        ret = {}
        ...
        if id is not False:
            ...
            for ds in self.datasources:
                ...
                for key, value in params.items():
                    ...
                    if key == 'TYPENAME':
                        action.layer = value[value.index(':')+1:]
